...
class PortfolioView:
    ...
    def __init__(self, store: AccountStore, topstepx_client=None) -> None:
        self._store = store
        self._live: Live | None = None
        self._balance_history: dict[str, deque[tuple[str, float]]] = {}
        self._topstepx_client = topstepx_client
        self._topstepx_accounts: list = []
        self._last_topstepx_fetch: float = 0.0
        self._topstepx_fetch_interval: float = 5.0  # Fetch every 5 seconds
    ...
    def rerender(self) -> None:
        if self._live is not None:
            ...
            # Fetch TopstepX accounts periodically (if event loop is running)
            try:
                loop = asyncio.get_running_loop()
                loop.create_task(self._refresh_topstepx_accounts())
            except RuntimeError:
                pass  # No event loop running
            self._live.update(self._build_layout(), refresh=True)
    
    async def _refresh_topstepx_accounts(self) -> None:
        """Fetch TopstepX accounts if client is configured and interval has elapsed."""
        if self._topstepx_client is None:
            return
        
        now = time.time()
        if now - self._last_topstepx_fetch < self._topstepx_fetch_interval:
            return
        
        try:
            self._topstepx_accounts = await self._topstepx_client.get_accounts()
            self._last_topstepx_fetch = now
        except Exception as e:
            logger.error(f"Error fetching TopstepX accounts: {e}")
    ...
    def _build_header(self) -> Panel:
        ...
        # TopstepX status
        topstepx_status = ""
        if self._topstepx_client:
            if self._topstepx_accounts:
                topstepx_status = f"  [dim]|  TopstepX: {len(self._topstepx_accounts)} accounts[/]"
            else:
                topstepx_status = "  [dim]|  TopstepX: connecting...[/]"
        
        return Panel(
            Text.from_markup(
                "[bold cyan]Portfolio Dashboard[/]  [bold red]●[/] "
                f"[bold green]LIVE[/]  [dim]|  {now}[/]{topstepx_status}"
            ),
            style="cyan",
            height=3,
        )
    ...
    def _build_summary_cards(self) -> Columns:
        ...
        # Add TopstepX accounts
        for i, ts_account in enumerate(self._topstepx_accounts, start=1):
            total_pnl = sum(pos.unrealized_pnl for pos in ts_account.positions)
            ...
            card = Panel(
                Text.from_markup(
                    f"[blue]TopstepX[/]\n"
                    f"[magenta]Equity:[/] ${ts_account.equity:,.2f}\n"
                    f"[yellow]Positions:[/] {len(ts_account.positions)}  "
                    f"[{pnl_color}]P&L:[/] [{pnl_color}]{pnl_sign}${total_pnl:,.2f}[/]"
                ),
                title=f"[bold]{ts_account.name[:20]}[/]",
                border_style="blue",
            )
            cards.append(card)
    ...
    def _build_positions_table(self) -> Panel:
        ...
        has_any_accounts = bool(accounts) or bool(self._topstepx_accounts)
        ...
        # Add TopstepX accounts
        for ts_account in self._topstepx_accounts:
            if accounts:  # Add section divider if there were simulated accounts
                table.add_section()
            
            account_name = f"[blue]{ts_account.name[:25]}[/]"
            total_pnl = sum(pos.unrealized_pnl for pos in ts_account.positions)
            
            # Account header row
            table.add_row(
                account_name,
                str(ts_account.trade_count),
                f"[green]${ts_account.balance:,.2f}[/]",
                "",
                "",
                "",
                "",
            ...
            if not ts_account.positions:
                ...
                for pos in ts_account.positions:
                    ...
                    table.add_row(
                        ...
                        "",
                        "",
                        pos.symbol[:20],  # Truncate long contract IDs
                        f"[{qty_color}]{pos.quantity:+.0f}[/]",
                        f"${pos.avg_price * abs(pos.quantity):,.2f}",
...
# Try to initialize TopstepX client if credentials are available
_topstepx_client = None
try:
    _jwt_token = os.getenv("TOPSTEPX_JWT_TOKEN")
    if _jwt_token:
        from topstepx_account import TopstepXAccountClient
        _topstepx_client = TopstepXAccountClient(
    ...
    logger.warning(f"TopstepX client not initialized: {e}")

price_book = PriceBook()
store = AccountStore(price_book)
view = PortfolioView(store, topstepx_client=_topstepx_client)
